...
public:
  ...
  inline void
  ApplyControlNotGateToQubits(const std::vector<size_t> &target_qubits) {
    Kokkos::MDRangePolicy<Kokkos::Rank<2>> policy(
        {0, 0}, {batch_size_, tableau_width_});

    for (size_t i = 0; i < target_qubits.size(); i += 2) {
      Kokkos::parallel_for(
          policy, BatchControlNotGateFunctor<Precision>(
                      *x_, *z_, *r_, target_qubits[i], target_qubits[i + 1]));
    }
  }
  ...
  inline void
  ApplyControlPhaseGateToQubits(const std::vector<size_t> &target_qubits) {
    Kokkos::MDRangePolicy<Kokkos::Rank<2>> policy(
        {0, 0}, {batch_size_, tableau_width_});

    for (size_t i = 0; i < target_qubits.size(); i += 2) {
      Kokkos::parallel_for(
          policy, BatchControlPhaseGateFunctor<Precision>(
                      *x_, *z_, *r_, target_qubits[i], target_qubits[i + 1]));
    }
  }
   ...
   * @param target_qubit Index of the target qubit.
   */
  inline void ApplyPhaseGateToQubits(const std::vector<size_t> &target_qubits) {
