  ...
  private:
    struct Measurement {
        ...
        void CopyToHost() {
            ...
            Kokkos::deep_copy(
                UnmanagedHostVectorView(measurement_determined_host.data(),
                                        batch_size),
                *measurement_determined_device);
        }
    };
  ...
  public:
    ...
    CliffordState(size_t num_qubits, size_t batch_size, int seed = -1,
        ...
        if (num_qubits_ > 0 and batch_size_ > 0) {
            x_ = std::make_unique<KokkosMat3D>("x_", batch_size_,
                                               tableau_width_, num_qubits_);

            z_ = std::make_unique<KokkosMat3D>("z_", batch_size_,
                                               tableau_width_, num_qubits_);

            r_ = std::make_unique<KokkosMat2D>("r_", batch_size_,
                                               tableau_width_);
        ...
        if (seed >= 0) {
            ...
            rand_pool_ = std::make_unique<
                Kokkos::Random_XorShift64_Pool<KokkosExecSpace>>(seed);
        } else {
            seed_ = 213434232223;
            rand_pool_ = std::make_unique<
                Kokkos::Random_XorShift64_Pool<KokkosExecSpace>>(seed_);
        }
    }
    ...
    inline void ApplyHadamardGateWithProb(size_t target_qubit, float prob) {
        ...
        Kokkos::MDRangePolicy<Kokkos::Rank<2>> policy(
            {0, 0}, {batch_size_, tableau_width_});
        Kokkos::parallel_for(
            policy, BatchHadamardGateWithProbFunctor<Precision>(
    ...
    inline void ApplyPhaseGateWithProb(size_t target_qubit, float prob) {
        ...
        Kokkos::MDRangePolicy<Kokkos::Rank<2>> policy(
            {0, 0}, {batch_size_, tableau_width_});
        Kokkos::parallel_for(
            policy, BatchPhaseGateWithProbFunctor<Precision>(
                        *x_, *z_, *r_, target_qubit, *rand_pool_, prob));
    }
    ...
    inline void ApplyPauliXGateWithProb(size_t target_qubit, float prob) {
        ...
        Kokkos::MDRangePolicy<Kokkos::Rank<2>> policy(
            {0, 0}, {batch_size_, tableau_width_});
        Kokkos::parallel_for(
            policy, BatchPauliXGateWithProbFunctor<Precision>(
                        *x_, *z_, *r_, target_qubit, *rand_pool_, prob));
    }
    ...
    inline void ApplyPauliZGateWithProb(size_t target_qubit, float prob) {
        ...
        Kokkos::MDRangePolicy<Kokkos::Rank<2>> policy(
            {0, 0}, {batch_size_, tableau_width_});
        Kokkos::parallel_for(
            policy, BatchPauliZGateWithProbFunctor<Precision>(
                        *x_, *z_, *r_, target_qubit, *rand_pool_, prob));
    }
    ...
    inline void HostToDevice(std::vector<Precision> &x,
        ...
        Kokkos::deep_copy(*r_, UnmanagedHostMat2DView(r.data(), r_->extent(0),
                                                      r_->extent(1)));
    }
    ...
    auto DeviceToHost() {
        ...
        Kokkos::deep_copy(UnmanagedHostMat3DView(z.data(), z_->extent(0),
                          ...
                          *z_);
        Kokkos::deep_copy(
            UnmanagedHostMat2DView(r.data(), r_->extent(0), r_->extent(1)),
            *r_);
