...
template <class Precision> class CliffordState {
  ...
  private:
    /**
     * @struct Measurement
     * @brief Holds quantum measurement results for a specific qubit, both on
  ...
  public:
        ...
        if (num_qubits_ > 0 and batch_size_ > 0) {
            ...
            if (init_to_zero_state) {
                ...
                Kokkos::parallel_for(policy,
                                     InitTableauToZeroStateFunctor<Precision>(
    ...
    CliffordState(std::vector<Precision> &x, std::vector<Precision> &z,
                  std::vector<Precision> &r, size_t num_qubits,
                  size_t batch_size, long int seed = -1,
                  const Kokkos::InitializationSettings &kokkos_args = {})
     ...
     */
    inline void HostToDevice(std::vector<Precision> &x,
        ...
        Kokkos::View<Precision **, Kokkos::HostSpace> r_host(
            "r_host", batch_size_, tableau_width_);
    ...
    /**
     * @brief Resets the quantum state to its initial state, characterized by
     * all qubits in the |0> state.
     */
    void ResetCliffordState() {
        if (num_qubits_ > 0 and batch_size_ > 0) {
            ...
            Kokkos::parallel_for(
                policy, InitTableauToZeroStateFunctor<Precision>(*x_, *z_, *r_,
                                                                 num_qubits_));
        }
    }
    ...
    inline auto CheckSum(int starting_batch_id = 0, int ending_batch_id = -1)
        ...
        Kokkos::MDRangePolicy<Kokkos::Rank<3>> policy(
            ...
            {ending_batch_id < 0 ? batch_size_ : ending_batch_id,
             tableau_width_, num_qubits_});
    ...
    ApplyPhaseGateWithApplyFlag(size_t target_qubit,
        ...
        Kokkos::MDRangePolicy<Kokkos::Rank<2>> policy(
            {0, 0}, {batch_size_, tableau_width_});
        Kokkos::parallel_for(policy,
                             BatchPhaseGateWithApplyFlagFunctor<Precision>(
                                 *x_, *z_, *r_, target_qubit, apply_flag));
    }
     ...
     * @param target_qubit Index of the target qubit.
     */
    inline void ApplyControlPhaseGate(size_t control_qubit,
                                      size_t target_qubit) {
     ...
     */
    inline void
    ApplyPhaseGateToQubits(const std::vector<size_t> &target_qubits) {

        Kokkos::MDRangePolicy<Kokkos::Rank<2>> policy(
            {0, 0}, {batch_size_, tableau_width_});

        for (size_t i = 0; i < target_qubits.size(); i++) {
            Kokkos::parallel_for(policy, BatchPhaseGateFunctor<Precision>(
                                             *x_, *z_, *r_, target_qubits[i]));
        }
    }

    inline void ApplyPhaseGate(size_t target_qubit) {
        ApplyPhaseGateToQubits({target_qubit});
    }
     ...
     */
    inline void
    ApplyPauliXGateToQubits(const std::vector<size_t> &target_qubits) {

        Kokkos::MDRangePolicy<Kokkos::Rank<2>> policy(
            {0, 0}, {batch_size_, tableau_width_});

        for (size_t i = 0; i < target_qubits.size(); i++) {
            Kokkos::parallel_for(policy, BatchPauliXGateFunctor<Precision>(
                                             *x_, *z_, *r_, target_qubits[i]));
        }
    }

    inline void ApplyPauliXGate(size_t target_qubit) {
        return ApplyPauliXGateToQubits({target_qubit});
    }
     ...
     */
    inline void
    ApplyPauliYGateToQubits(const std::vector<size_t> &target_qubits) {
        Kokkos::MDRangePolicy<Kokkos::Rank<2>> policy(
            {0, 0}, {batch_size_, tableau_width_});

        for (size_t i = 0; i < target_qubits.size(); i++) {
            Kokkos::parallel_for(policy, BatchPauliYGateFunctor<Precision>(
                                             *x_, *z_, *r_, target_qubits[i]));
        }
    }

    inline void ApplyPauliYGate(size_t target_qubit) {
        return ApplyPauliYGateToQubits({target_qubit});
    }
     ...
     */
    inline void
    ApplyPauliZGateToQubits(const std::vector<size_t> &target_qubits) {

        Kokkos::MDRangePolicy<Kokkos::Rank<2>> policy(
            {0, 0}, {batch_size_, tableau_width_});

        for (size_t i = 0; i < target_qubits.size(); i++) {
            Kokkos::parallel_for(policy, BatchPauliZGateFunctor<Precision>(
                                             *x_, *z_, *r_, target_qubits[i]));
        }
    }

    inline void ApplyPauliZGate(size_t target_qubit) {
        ApplyPauliZGateToQubits({target_qubit});
    }
     ...
     * @param measurement_id Optional identifier for the measurement, if absent
     * a new measurement is created.
     * @return The measurement ID associated with this measurement operation.
     */
    auto MeasureQubits(const std::vector<size_t> &target_qubits,
                       float bias = 0.5) {

        for (size_t i = 0; i < target_qubits.size(); i++) {
            MeasureQubit(target_qubits[i], bias);
        }

        // Kokkos::RangePolicy<Kokkos::DefaultExecutionSpace> policy(0,
        // batch_size_); std::vector<size_t>
        // measurement_ids(target_qubits.size());

        // for (size_t i = 0; i < target_qubits.size(); i++) {
        //   measurements_.push_back(Measurement());
        //   measurement_ids.push_back(measurements_.size() - 1);
        //   measurements_.back().qubit_measured = target_qubits[i];
        //   std::string qubit_name = "q" + std::to_string(target_qubits[i]);
        //   std::string measurement_name =
        // 	"m" + std::to_string(measurement_ids[i]);
        ...
        //                            *x_, *z_, *r_,
        //                            *(measurements_[measurement_ids[i]]
        //                                  .measurement_results_device),
        //                            *(measurements_[measurement_ids[i]]
        //                                  .measurement_determined_device),
        //                            *rand_pool_, target_qubits[i], bias));
        // }
    ...
    size_t MeasureQubit(size_t target_qubit, float bias = 0.5,
        ...
        if (!measurement_id.has_value()) {
            ...
            std::string measurement_name =
                "m" + std::to_string(*measurement_id);
            std::string results_name =
                qubit_name + "_" + measurement_name + "_results";
            std::string determined_name =
                qubit_name + "_" + measurement_name + "_determined";
        ...
        Kokkos::parallel_for(policy, BatchMeasureFunctor<Precision>(
                                         *x_, *z_, *r_,
    ...
    auto GetMeasurement(size_t measurement_id, size_t batch_index) {
        ...
        if (measurement_id < measurements_.size() and
            batch_index < batch_size_) {
            measurements_[measurement_id].CopyToHost();

            // std::cout << "measurements_[measurement_id].qubit = " <<
            // measurements_[measurement_id].qubit_measured << std::endl;
            result =
                std::make_pair(measurements_[measurement_id]
                                   .measurement_results_host[batch_index],
    ...
    auto &GetMeasurement(size_t measurement_id) {
        PLAQUETTE_ABORT_IF_NOT(
            measurement_id < measurements_.size(),
