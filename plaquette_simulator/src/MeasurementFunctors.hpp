...
KOKKOS_INLINE_FUNCTION Precision RowProductSign(Kokkos::View<Precision ***> x,
    ...
    for (size_t j = 0; j < num_qubits; ++j) {
        ...
        // std::cout << "x(batch_id, i, j) = " << x(batch_id, i, j) <<
        // std::endl; std::cout << "z(batch_id, i, j) = " << z(batch_id, i, j)
        // << std::endl; std::cout << "x(batch_id, k, j) = " << x(batch_id, k,
        // j) << std::endl; std::cout << "z(batch_id, k, j) = " << z(batch_id,
        // k, j) << std::endl;
    }
...
KOKKOS_INLINE_FUNCTION Precision MeasureRandom(
    ...
    // std::cout << "CheckSumInline(x,z,r) 0 = " << CheckSumInline(x,z,r) <<
    // std::endl;

    CopyFromRowAtoRowB(x, z, r, batch_id, p + num_qubits, p);
    // std::cout << "CheckSumInline(x,z,r) 1 = " << CheckSumInline(x,z,r) <<
    // std::endl;
    ZeroRow(x, z, r, batch_id, p + num_qubits);

    z(batch_id, p + num_qubits, target_qubit) = 1;

    // std::cout << "CheckSumInline(x,z,r) 2 = " << CheckSumInline(x,z,r) <<
    // std::endl;

    auto rand_gen = rand_pool.get_state();
    float rand_val = rand_gen.frand(0.0, 1.0);
    r(batch_id, p + num_qubits) = rand_val < bias;
    // std::cout << "r(batch_id, p + num_qubits) = " << r(batch_id, p +
    // num_qubits)
    // << std::endl;
    rand_pool.free_state(rand_gen);

    // std::cout << "CheckSumInline(x,z,r) 3 = " << CheckSumInline(x,z,r) <<
    // std::endl;
    ...
    // std::cout << "CheckSumInline(x,z,r) 4 = " << CheckSumInline(x,z,r) <<
    // std::endl;
...
template <class Precision> struct BatchMeasureFunctor {
    ...
    BatchMeasureFunctor(Kokkos::View<Precision ***> x,
        ...
        : r_(r), x_(x), z_(z), measurement_result_(measurement_result),
          ...
          rand_pool_(rand_pool), target_qubit_(target_qubit), bias_(bias) {
    ...
    void operator()(const size_t batch_id) const {
        ...
        for (size_t p = 0; p < num_qubits_; ++p) {
            ...
            if (x_(batch_id, p + num_qubits_, target_qubit_)) {
                ...
                // std::cout << "target_qubit_ = " << target_qubit_ <<
                // std::endl;
