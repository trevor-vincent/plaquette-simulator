...
PYBIND11_MODULE(plaquette_simulator_bindings, m) {
    ...
    py::class_<CliffordState<int>>(m, "CliffordState")
        ...
        .def("apply_hadamard_gate_with_prob",
             &CliffordState<int>::ApplyHadamardGateWithProb)
        .def("apply_control_not_gate_with_prob",
             &CliffordState<int>::ApplyControlNotGateWithProb)
        .def("apply_control_phase_gate_with_prob",
             &CliffordState<int>::ApplyControlPhaseGateWithProb)
        .def("apply_phase_gate_with_prob",
             &CliffordState<int>::ApplyPhaseGateWithProb)
        .def("apply_pauli_x_gate_with_prob",
             &CliffordState<int>::ApplyPauliXGateWithProb)
        .def("apply_pauli_z_gate_with_prob",
             &CliffordState<int>::ApplyPauliZGateWithProb);
}
} // namespace
