...
TEMPLATE_TEST_CASE("CliffordStateKokkos::CopyConstructor",
...

  {
    ...
    CHECK(kokkos_state_1.getLength() == kokkos_state_2.getLength());
    CHECK(kokkos_state_1.getNumQubits() == kokkos_state_2.getNumQubits());

    std::vector<Kokkos::complex<TestType>> kokkos_state_1_host(
							       kokkos_state_1.getLength());

    std::vector<Kokkos::complex<TestType>> kokkos_state_2_host(
