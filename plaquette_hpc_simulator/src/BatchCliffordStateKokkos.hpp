...
public:
  ...
  BatchCliffordStateKokkos(size_t num_qubits, size_t batch_size,
    ...
    if (num_qubits_ > 0 and batch_size_ > 0) {
      ...
      Kokkos::MDRangePolicy<Kokkos::Rank<3>> policy({0, 0, 0}, {batch_size_, tableau_width_, tableau_width_});
      Kokkos::parallel_for(policy, InitTableauToZeroState<Precision>(*x_, *z_, *r_, num_qubits_));
    }
  }
  ...
  inline void ApplyHadamardGate(size_t target_qubit) {
    ...
    Kokkos::MDRangePolicy<Kokkos::Rank<2>> policy({0, 0}, {batch_size_, tableau_width_});
    
    Kokkos::parallel_for(
			 policy,
			 BatchHadmardGateFunctor<Precision>(*x_, *z_,
							    *r_,
							    target_qubit));
  }
