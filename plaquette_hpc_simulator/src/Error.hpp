 ...
 */
#define PLAQUETTE_ABORT(message) Plaquette::Abort(message, __FILE__, __LINE__, __func__)
/**
 ...
 *
 * This function should not be called directly - use one of the `PLAQUETTE_ASSERT()`
 * or `PLAQUETTE_ABORT()` macros, which provide the source location at compile time.
 *
 ...
 */
[[noreturn]] inline void Abort(const char *message, const char *file_name, int line,
                  const char *function_name)
{
    std::stringstream err_msg;
    err_msg << "[" << file_name << "][Line:" << line
            << "][Method:" << function_name << "]: Error in Plaquette: " << message;
