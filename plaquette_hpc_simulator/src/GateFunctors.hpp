...
template <class Precision> struct HadamardGateFunctor {
  ...
  HadamardGateFunctor(Kokkos::View<Precision *> &r,
                      Kokkos::View<Precision *> &x,
                      Kokkos::View<Precision *> &z, std::size_t num_qubits,
                      std::size_t target_qubit)
      : r_(r), x_(x), z_(z), target_qubit_(target_qubit) {
    num_x_or_z_rows_ = 2 * num_qubits + 1;
  ...
  KOKKOS_INLINE_FUNCTION
  void operator()(const std::size_t bit) const {
    // Set r = r ^ (x_target_qubit_*z_target_qubit_)
    r[bit] ^= x[bit * num_x_or_num_x_or_z_rows_ + target_qubit_] *
              z[bit * num_x_or_z_rows_ + target_qubit_];
    // Swap x_target and z_target
    KE::swap(x[bit * num_x_or_num_x_or_z_rows_ + target_qubit_],
             z[bit * num_x_or_z_rows_ + target_qubit_]);
  }
};
